...
void drawall(char* f1, char* f2 ,char* label1, char* label2, char* var , string outname){
  ...
  const int npaths = 1;
  string paths[npaths]={"electronOfETau"};
  string Histo_paths[npaths];
  for(int i=0;i<npaths;i++){Histo_paths[i]=paths[i]+"DQM/total eff";cout<<Histo_paths[i]<<endl;}
  ...
  TFile* f=new TFile("ElectronOfETauPlots.root","RECREATE");
